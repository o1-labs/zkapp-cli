...
import { pagesIndex, appIndex } from '../lib/ui/next/customNextIndex.js';
...
async function scaffoldNext(projectName) {
  ...
  fs.copySync(
    path.join(__dirname, 'ui', 'next', 'next.config.js'),
    path.join('ui', 'next.config.js')
  );

  let indexFileName;
  if (useAppDir) {
    indexFileName = useTypescript ? 'page.tsx' : 'page.js';
  } else {
    indexFileName = useTypescript ? 'index.tsx' : 'index.js';
  }

  fs.writeFileSync(
    path.join('ui', `src/${useAppDir ? 'app' : 'pages'}`, indexFileName),
    useAppDir ? appIndex : pagesIndex,
    'utf8'
  );

  // Update layout file for working with app directory
  if (useAppDir) {
    // Find Head component and update title and description
    const layoutFileName = useTypescript ? 'layout.tsx' : 'layout.js';

    // Update Head
    const appLayout = fs.readFileSync(
      path.join('ui', 'src', 'app', layoutFileName),
      'utf8'
    );

    let newAppLayout = appLayout.replace(
      "title: 'Create Next App'",
      "title: 'Mina zkApp UI'"
    );
    newAppLayout = newAppLayout.replace(
  ...
  if (useGHPages) {
    ...
    console.log(
      'Using project name ' +
        projectName +
        ' for GitHub repo name. Change in next.config.js and ["pages" or "app"]/reactCOIServiceWorker.tsx if this is not correct or changes'
    );
  ...
  pageExtensions: ['page.tsx', 'page.ts', 'page.jsx', 'page.js'],`
    ...
    let wrapperFileName;
    let wrapperPagesFileName;
    let indexFileName;
    let indexPagesFileName;
    ...
    if (useAppDir) {
      wrapperFileName = useTypescript ? 'layout.tsx' : 'layout.js';
      wrapperPagesFileName = useTypescript
        ? 'layout.page.tsx'
        : 'layout.page.js';
      indexFileName = useTypescript ? 'page.tsx' : 'page.js';
      indexPagesFileName = useTypescript ? 'page.page.tsx' : 'page.page.js';
    } else {
      wrapperFileName = useTypescript ? '_app.tsx' : '_app.js';
      wrapperPagesFileName = useTypescript ? '_app.page.tsx' : '_app.page.js';
      indexFileName = useTypescript ? 'index.tsx' : 'index.js';
      indexPagesFileName = useTypescript ? 'index.page.tsx' : 'index.page.js';
    }

    shell.mv(
      path.join('ui', `src/${useAppDir ? 'app' : 'pages'}`, wrapperFileName),
      path.join(
        'ui',
        `src/${useAppDir ? 'app' : 'pages'}`,
        wrapperPagesFileName
      )
    );
    shell.mv(
      path.join('ui', `src/${useAppDir ? 'app' : 'pages'}`, indexFileName),
      path.join('ui', `src/${useAppDir ? 'app' : 'pages'}`, indexPagesFileName)
    );
